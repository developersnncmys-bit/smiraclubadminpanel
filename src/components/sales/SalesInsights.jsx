...
export default function SalesInsights({ rows, bookings, team, activeStatus, onPickStatus, actions }) {
  ...
  const clickable = Boolean(onPickStatus);
  ...
  const hint = {
    funnel: clickable ? 'Click a stage to filter the list below' : 'Where every lead currently sits',
            ...
            <ul className="space-y-3">
              {enquiryStatuses.map((status) => {
                ...
                return (
                  <li key={status}>
                    <button
                      onClick={clickable ? () => onPickStatus(status) : undefined}
                      className={`w-full rounded-xl px-3.5 py-2.5 text-left transition ${
                        on ? 'bg-brand-50 ring-1 ring-brand-600/25' : clickable ? 'hover:bg-surface-soft' : 'cursor-default'
                      }`}
                    >
